...
// Matches the menu's `w-48` below — needed up front to compute `left`
// before the menu itself has been measured.
...
export function TenantActionsMenu({
  ...
  function toggle() {
    if (!open && buttonRef.current) {
      ...
      setMenuPosition({ top: buttonRect.bottom + 4, left: buttonRect.right - MENU_WIDTH });
    }
    setOpen((value) => !value);
  }

  // The initial position (above) is only a guess based on the button, made
  // before the menu has rendered. Once it's in the DOM we know its real
  // height, so flip it above the button if it would otherwise run off the
        ...
        createPortal(
          <>
            ...
            <div
              ...
              className="fixed z-20 rounded-md border border-venturo-olive/15 bg-white py-1 shadow-lg"
            >
