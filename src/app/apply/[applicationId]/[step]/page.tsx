...
export default async function ApplicationStepPage({
  ...
  searchParams: Promise<{ error?: string; missing?: string; message?: string }>;
}) {
  const dbUser = await requireVerifiedUser();
  const { applicationId, step } = await params;
  const { error, missing, message } = await searchParams;
  ...
  return (
    <Card>
      ...
      {error === "upload-failed" && (
        <p className="mb-6 rounded-md bg-red-50 px-3 py-2.5 text-sm text-red-700">
          {message || "That file couldn't be uploaded. Please try a different file."}
        </p>
      )}
